...
- **UI:**
  ...
  - Кнопки паузы/меню, всплывающий оверлей меню
  - Панель строительства снизу
- **Архитектура:** сервис-контейнер (`GameRuntime`) + чистые FSM на XState (глобальная `GameStateMachine`, `WorkerStateMachine`)
- **Тесты:** Vitest для FSM
...
### FSM игры
`src/core/GameStateMachine.js` описывает переходы MENU → LOADING → PLAYING → PAUSED. `GameScene` дергает события, UI паузы/меню изменяет состояние.
