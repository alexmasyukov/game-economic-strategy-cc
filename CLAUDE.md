...
### Основной функционал
- ✅ Сетка 40x40 клеток (25x25 пикселей каждая)
- ✅ Полноэкранный режим с адаптивным масштабированием
- ✅ Pathfinding через EasyStar.js - рабочие обходят здания
- ✅ Визуализация пути - полупрозрачные линии показывают маршрут рабочего
- ✅ State machine для рабочих (IDLE, PRODUCING, CARRYING, RETURNING)
- ✅ Автоматическое назначение рабочих на новые здания
- ✅ Управление скоростью игры (x1, x2, x3, x5, x7)
- ✅ UI: панель ресурсов сверху, панель строительства снизу

### Стартовая сцена
- Замок 3x3 по центру
- Склад 2x2 рядом с замком
- Очаг 1x1 с другой стороны от замка
- 5 рабочих около очага
...
- `findFreeBuildingSpot(centerX, centerY, width, height)` - находит свободное место для строительства
...
1. **Назначение:** UI создает теплицу → BuildingManager назначает свободного рабочего
2. **Путь к зданию:** Worker получает путь через PathfindingManager
3. **PRODUCING:** Рабочий прибывает → Greenhouse.startProduction()
4. **Ожидание:** Worker ждет 2 секунды (производство помидора)
5. **Сбор:** Greenhouse устанавливает `resourceReady=true`
6. **CARRYING:** Worker собирает ресурс → ищет путь к Storage
7. **Доставка:** Worker прибывает на склад → Storage.receiveResource()
8. **RETURNING:** Worker ищет путь обратно к теплице
9. **Цикл повторяется** с шага 3
...
### Координаты
- **Grid координаты:** целые числа (0-39 для сетки 40x40)
- **World координаты:** пиксели (gridToWorld возвращает центр клетки)
...
4. Добавить кнопку в `UIManager.createBottomBar()`
...
- Размер сетки фиксированный (40x40)
...
- [ ] Добавить возможность удаления зданий
- [ ] Добавить сохранение/загрузку игры
- [ ] Добавить звуки
- [ ] Добавить спрайты вместо примитивов
