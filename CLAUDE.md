# Village Builder — краткая техническая документация

## Обзор
Web-игра на Phaser 3.90 с системой ресурсов и рабочих. Основные системы связаны через `GameRuntime` (сервис-контейнер на сцене), поиск пути — EasyStar.js, FSM рабочих — XState, сборка — Vite, тесты — Vitest.

## Архитектура
```
src/
├─ config/            # Константы, конфиг Phaser
├─ core/              # Runtime и чистые FSM
├─ managers/          # Grid, Pathfinding, Resource, Building, Worker, Placement, UI
├─ entities/          # Building/ProductionBuilding/Ghost/Worker + конкретные здания
├─ scenes/            # GameScene
└─ main.js            # Точка входа
```

### Runtime
`GameRuntime` создаёт менеджеры и пробрасывает их в сцену как `scene.services`. Он же управляет множителем времени и начальными постройками/рабочими.

### Ключевые системы
- **GridManager** — сетка занятости, конвертация world/grid, отрисовка сетки/секторов.
- **PathfindingManager** — EasyStar, диагонали и corner cutting, поиск к точке рядом со зданием.
- **ResourceManager** — хранение ресурсов, оповещение UI.
...
## Запуск
- Dev: `npm start` (Vite, порт 8000)
- Build: `npm run build`
- Tests: `npm test` (Vitest, чистая FSM рабочих)
